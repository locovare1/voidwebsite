  ...
  return (
    ...
    >
      <div
        ...
        className="relative w-full max-w-4xl bg-[#0a0a0a] border border-white/10 rounded-3xl overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-300 my-8"
      >
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        {article.image && (article.image.startsWith('/') || article.image.startsWith('http://') || article.image.startsWith('https://')) && (
          <div className="relative w-full h-64 sm:h-80 lg:h-96 bg-[#050505] overflow-hidden">
            <Image
              src={article.image}
              alt={article.title}
              fill
